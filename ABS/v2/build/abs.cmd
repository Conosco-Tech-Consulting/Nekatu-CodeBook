...
for /f "tokens=3" %%a in ('reg query "hklm\software\Joskos Solutions\ABS" /v BackupSharepointHost ^| find "BackupSharepointHost"') do set BckpSharepointHost=%%a

if /i {%1}=={/test} (
shift
set BckpSrc=SystemState
) ELSE (
set BckpSrc=%ABSDir%\Jobs\Job1.dat
)

for /l %%a in (1,1,254) do if /i {%1}=={/job:%%a} (
shift
set BckpSrc=%ABSDir%\Jobs\Job%%a.dat
set BckpSrcEnc=%ABSDir%\Jobs\Job%%a.bks
) ELSE (
set BckpSrc=%ABSDir%\Jobs\Job1.dat
set BckpSrcEnc=%ABSDir%\Jobs\Job1.bks
)
...
if /i "%BckpTapeDst%"=="8mm" set BckpTapeDst=8mm AIT1
...
IF "%OSVer%"=="Version.5" SET BckpParams1=/j "%BckpDate%" /f "%BckpFileDst%" /V:yes /L:f /M normal
...
IF "%OSVer%"=="Version.5" SET BckpParams2=/j "Mail-%BckpDate%" /f "%BckpFileDst%" /A /V:yes /L:f /M normal
...
if /i "%sharepoint%"=="0x1" "%systemdrive%\Program Files\Common Files\Microsoft Shared\web server extensions\60\BIN\stsadm.exe" -o backup -url "%BckpSharepointHost%" -filename "%BckpSharepoint%" -overwrite


if /i "%BckpSrc%"=="SystemState" (
ntbackup backup %BckpSrc% %BckpParams1%
) else (
redshirt /d "%BckSrcEnc%" "%BckSrc%"
ntbackup backup @%BckpSrc% %BckpParams1%
del "%BckSrc%" /q
)
 
if exist "%absdir%\jobs\Job0.dat" (
redshirt /d "%absdir%\jobs\Job0.dat" "%absdir%\jobs\Job0.bks"
ntbackup backup "%absdir%\mail.bks" %BckpParams1%
del "%absdir%\jobs\job0.bks" /q
)
