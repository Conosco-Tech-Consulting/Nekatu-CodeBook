...
strThisPlaylist = "c:\Hour" & wscript.arguments(1) & ".mp3"
strNextPlaylist = "c:\Hour" & wscript.arguments(2) & ".mp3"
strDestPath = objFSO.GetParentFolderName(strFile)
strDestFile = objFSO.GetBaseName(strFile) & "." & wscript.arguments(1) & ".tmp"
While Not objFile.AtEndOfStream
	strLine = objFile.ReadLine
	If Left(strLine, 20) = "c:\Hour" & wscript.arguments(1) & ".mp3" Then
		strThisPlaylist = Mid(strLine, 21)
	End If
	If Left(strLine, 20) = "c:\Hour" & wscript.arguments(2) & ".mp3" Then
		wscript.quit
	End If
