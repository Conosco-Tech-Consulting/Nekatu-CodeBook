...
:START
title m3u Playlist Splitter - v1.07
echo ======================================
echo  .-._.-._.-. .----. .-. .-. ----------
echo  : .-. .-. : '--. : : : : :  PLAYLIST
echo  : : : : : : .--' : : : : :  SPLITTER
...
echo.>"%~dp1%~n1.0%%a.tmp"
call :SPLIT %1 0%%a
for /f %%b in ('type "%~dp1%~n1.0%%a.tmp" ^| find /c /i "C:\Q\Powergold4\Hour0%%a.mp3"') do if not "%%b"=="0" (
		type "%~dp1%~n1.0%%a.tmp" | find /v /i "C:\Q\Powergold4\Hour0%%a.mp3" | find /i ".mp3">"%~dp1%~n1.0%%a.m3u"
		del "%~dp1%~n1.0%%a.tmp"
	) else (
		del "%~dp1%~n1.0%%a.tmp"
	)
...
echo.>"%~dp1%~n1.%%a.tmp"
call :SPLIT %1 %%a
for /f %%b in ('type "%~dp1%~n1.%%a.tmp" ^| find /c /i "C:\Q\Powergold4\Hour%%a.mp3"') do if not "%%b"=="0" (
		type "%~dp1%~n1.%%a.tmp" | find /v /i "C:\Q\Powergold4\Hour%%a.mp3" | find /i ".mp3">"%~dp1%~n1.%%a.m3u"
		del "%~dp1%~n1.%%a.tmp"
	) else (
		del "%~dp1%~n1.%%a.tmp"
	)
...
if "%2"=="23" cscript //nologo %systemroot%\system32\m3usplit.vbs %1 23 00
