' ----------------------------------------------------------
	strApp = "Joskos Solutions Server Monitor"
	strAppVer = "v2.3"
...
Dim objPrinter, colPrinters, strPrinter, strWMIQuery
...
if strMode = "" Then
	strMode = InputBox("What operation would you like to run?" &_
			...
			vbCrLf & "T - Test RDP logon to specified server" &_
			vbCrLf & "L - Printer check on a remote/local server" &_
			vbCrLf & "I - Gather System Information on specified server" &_
			vbCrLf & "R - Upload System Information to SharePoint" &_
			vbCrLf &_
			vbCrLf & "Choices: D, C, P, S, Q, T, I, R",strApp)
...
'		SendAlert strFrom,strTo,strSubject,strBody,2
		...
		RecordAlert StrCheck,StrResult
	Else
		strResult = 1
		wscript.echo strCheck & ": " & StrResult
		RecordAlert StrCheck,StrResult
...
Sub RDPCheck(strHost,strUN,strPW,strFrom,strTo,strSubject,strBody,nPriority)

	strCheck = "RDP Check - " & strHost
	
	If Not TSLogon(strHost) Then
...
SPUpload "sysinfo.txt", "https://joskosgroup.sharepoint.com/support/internal/Shared%20Documents/Logs/" & strHome & " - " & strHost & ".htm" , "arikf@joskos.com", "lqdtqvjqnkfgbkny"
...
Sub RecordAlert(strCheck,strResult)
...
'	sServer="SP01\SHAREPOINT"
'	sConn="provider=sqloledb;data source=" & sServer & ";initial catalog=FusionTest"
'	Set oConn = CreateObject("ADODB.Connection")
'	oConn.Open sConn, "upload", "S3cur1ty"
'	Set oRS =CreateObject("ADODB.Recordset")
'	sSQL="insert into SvrMonitor(AlertSubject,AlertBody,AlertDate,AlertHost) values('" & strCheck & "','" & strResult & "',getdate(),'" & strHost & "')"
'	oRS.open sSQL, oconn
...
Sub DBQuery(strHost)
	...
	connstr="Provider=sqloledb;Data Source=fusion-sql2;Initial Catalog=monitoringdb;Integrated Security=SSPI"
	AdCn.Open = connstr
